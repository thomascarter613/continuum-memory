...
function evidenceCount(
  memory: Pick<MemoryRecord | MemoryCandidate, "sourceEventIds" | "sourceArtifactIds">,
) {
...
export function evaluateMemoryLike(input: {
  ...
  if (target.content.trim().length < 24) {
    score -= 0.18
    findings.push({
      code: "memory.content.too_short",
      severity: "warning",
      message: "Memory content is very short.",
    })
  ...
  if (sources === 0) {
    score -= 0.35
    findings.push({
      code: "memory.evidence.missing",
      severity: "error",
      message: "Memory has no source event or artifact evidence.",
    })
  ...
  if (target.confidence < 0.5) {
    score -= 0.16
    findings.push({
      code: "memory.confidence.low",
      severity: "warning",
      message: "Memory confidence is below 0.5.",
    })
  ...
  if (!target.namespace.includes(":")) {
    score -= 0.08
    findings.push({
      code: "memory.namespace.weak",
      severity: "info",
      message: "Namespace is not strongly typed.",
    })
  ...
  if (target.sensitivity === "secret") {
    score -= 0.35
    findings.push({
      code: "memory.sensitivity.secret",
      severity: "error",
      message: "Secret material should not be stored as ordinary memory.",
    })
    recommendations.push("Replace secret values with a redacted vault reference.")
  } else if (target.sensitivity === "sensitive") {
    score -= 0.1
    findings.push({
      code: "memory.sensitivity.sensitive",
      severity: "warning",
      message: "Sensitive memory requires stricter retrieval policy.",
    })
  ...
  if (!projectIdFromScope(target.scope) && target.namespace.startsWith("project:")) {
    score -= 0.08
    findings.push({
      code: "memory.scope.project_id_missing",
      severity: "info",
      message: "Project namespace does not include scope.projectId.",
    })
...
export function evaluateHandoffCompleteness(handoff: HandoffPack): CreateMemoryEvaluation {
  ...
  for (const [section, present] of requiredSections) {
    if (!present) {
      ...
      recommendations.push(
        `Populate ${section} before relying on this handoff for cross-session transfer.`,
      )
    }
  }
  ...
  if (!payload.contextPackId) {
    score -= 0.08
    findings.push({
      code: "handoff.context_pack.missing",
      severity: "info",
      message: "Handoff is not linked to a context pack.",
    })
    recommendations.push(
      "Prefer compiled handoffs so contextPackId and retrieval audit records are preserved.",
    )
  }
